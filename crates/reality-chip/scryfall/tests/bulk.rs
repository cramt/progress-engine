...
use chip_scryfall::bulk::{strip_reminder_text, Anomaly, BulkCard};
use chip_scryfall::index::{Index, IndexFile};
...
fn a_token_never_shadows_the_card_it_is_named_after() {
    ...
    assert_eq!(
        ...
        chip_scryfall::legality::CommanderLegality::Legal,
...
fn the_index_records_the_shape_it_was_written_in() {
    let index = built();
    assert_eq!(index.schema(), chip_scryfall::index::SCHEMA);
