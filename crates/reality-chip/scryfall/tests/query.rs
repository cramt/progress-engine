...
use chip_scryfall::index::{Card, Index, IndexFile, TagGap};
use chip_scryfall::legality::Legalities;
use chip_scryfall::{self as query, CardView, Colors, ParseError, Query};
...
fn written(cards: &[(&str, &[&str])]) -> IndexFile {
    let mut index = Index {
        schema: Some(chip_scryfall::index::SCHEMA),
        ..Index::default()
    };
    for (name, keywords) in cards {
        index.cards.insert(
            chip_scryfall::index::keyname(name),
            Card {
                name: (*name).into(),
