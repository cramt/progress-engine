...
use chip_scryfall::index::{Card, Index};
...
fn hits(index: &Index, query: &str) -> Vec<String> {
    let q = chip_scryfall::parse(query).unwrap_or_else(|e| panic!("{query:?}: {e}"));
...
fn french_vanilla_is_declined_rather_than_approximated() {
    assert!(matches!(
        chip_scryfall::parse("is:frenchvanilla"),
        Err(chip_scryfall::ParseError::UnknownIsProperty { .. })
    ));
}
...
fn mana_costs_compare_as_multisets_of_symbols() {
    ...
    assert_eq!(
        chip_scryfall::parse("m:3WW").unwrap(),
        chip_scryfall::parse("m:{3}{W}{W}").unwrap()
    );
...
fn a_hybrid_symbol_reads_the_same_written_either_way() {
    use chip_scryfall::mana::ManaCost;
...
fn a_devotion_term_naming_two_colours_is_refused() {
    assert!(matches!(
        chip_scryfall::parse("devotion:{u}{b}"),
        Err(chip_scryfall::ParseError::MixedDevotion { .. })
    ));
