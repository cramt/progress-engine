...
use gauntlet_criteria::{Answering, Grouping, LandDetail, Palette, Plan, Reading, Schedule};
use gauntlet_toml::{QuestionReads, Reads};
...
impl Shared {
    ...
    /// interchangeable to [`gauntlet_criteria::Cost`], and they are interchangeable
...
mod tests {
    use super::*;
    use gauntlet_criteria::Policies;

    fn reads_of(source: &str) -> QuestionReads {
        gauntlet_toml::Criteria::parse(source, "test")
            .expect("parses")
            .reads()
    }
